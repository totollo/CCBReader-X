...
static map<string, CCData*> _cache;

void CCBReader::cleanCache(){
    _cache.clear();
}
...
CCNode* CCBReader::readNodeGraphFromFile(const char *pCCBFileName, CCObject *pOwner, const CCSize &parentSize)
{
    ...
    CCData *data = _cache[strCCBFileName];
    if (data == NULL) {
...
CCNode* CCBReader::readNodeGraphFromData(CCData *pData, CCObject *pOwner, const CCSize &parentSize)
{
    ...
    CCNode *pNodeGraph = readFileWithCleanUp(false, animationManagers);
...
//        CCLog("class name: %s",className.c_str());
    }
...
CCBKeyframe* CCBReader::readKeyframe(int type)
{
    ...
    else if (type == kCCBPropTypeTimeLine){
        std::string timeline = readCachedString();
        float f = readFloat();
        value = CCArray::create(CCBValue::create(timeline.c_str()),CCBValue::create(f),NULL);
    }
