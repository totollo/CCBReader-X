...
void CCNodeLoader::parseProperties(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader) {
    ...
    for(int i = 0; i < propertyCount; i++) {
        ...
        switch(type)
        {
            ...
            case kCCBPropTypeTimeLine: {
                std::string str = this->parsePropTypeString(pNode, pParent, pCCBReader);
                float f = this->parsePropTypeFloat(pNode, pParent, pCCBReader);
//                CCLog("%s %f",str.c_str(),f);
                break;
            }
            case kCCBPropTypeSound:{
                std::string text = this->parsePropTypeSound(pNode, pParent, pCCBReader);
                if(setProp) {
                    this->onHandlePropTypeSound(pNode, pParent, propertyName.c_str(), text.c_str(), pCCBReader);
                }
                break;
            }
            default:
...
std::string CCNodeLoader::parsePropTypeSound(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader) {
    return pCCBReader->readCachedString();
}
...
CCNode * CCNodeLoader::parsePropTypeCCBFile(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader) {
    ...
    return CCBLoader::nodeFromCCBFile(ccbFileName.c_str());
...
void CCNodeLoader::onHandlePropTypeSound(CCNode * pNode, CCNode * pParent, const char* pPropertyName, const char * pText, CCBReader * pCCBReader) {
    ASSERT_FAIL_UNEXPECTED_PROPERTY(pPropertyName);
}
