...
, mOwner(NULL)
, completeAction(NULL)
, callbackChannelAction(NULL)
, soundChannelAction(NULL)
{
    init();
}
...
CCActionInterval* CCBAnimationManager::getAction(CCBKeyframe *pKeyframe0, CCBKeyframe *pKeyframe1, const char *pPropName, CCNode *pNode,bool last)
{
    //    printf("%s\n",pPropName);
    ...
    else if(strcmp(pPropName, "timeLine") == 0){
        CCArray *value1 = (CCArray*)pKeyframe0->getValue();
        CCArray *value2 = (CCArray*)pKeyframe1->getValue();
        
        string* str0 = new string(((CCBValue*)value1->objectAtIndex(0))->getStringValue());
        float f0 = ((CCBValue*)value1->objectAtIndex(1))->getFloatValue();
        
        string* str1 = new string(((CCBValue*)value2->objectAtIndex(0))->getStringValue());
        float f1 = ((CCBValue*)value2->objectAtIndex(1))->getFloatValue();
        
        CCLog("%s  %f",str0->c_str(),f0);
        CCLog("%s  %f\n",str1->c_str(),f1);
        
        if (last) {
            CCCallFuncND* action0 = CCCallFuncND::create(this, callfuncND_selector(CCBAnimationManager::switchToCCBTimeLine), str0);
            CCCallFuncND* action1 = CCCallFuncND::create(this, callfuncND_selector(CCBAnimationManager::switchToCCBTimeLine), str1);
            CCSpawn* action2 = CCSpawn::create(action0,CCDelayTime::create(duration),NULL);
            CCSequence* action = CCSequence::createWithTwoActions(action2, action1);
            return action;
        }else{
            CCCallFuncND* action0 = CCCallFuncND::create(this, callfuncND_selector(CCBAnimationManager::switchToCCBTimeLine), str0);
            CCSpawn* action2 = CCSpawn::create(action0,CCDelayTime::create(duration),NULL);
...
void CCBAnimationManager::switchToCCBTimeLine(cocos2d::CCNode *node,void* data){
    string* str = (string*)data;
    CCLog("switchToCCBTimeLine %s",str->c_str());
    CCBAnimationManager* am = (CCBAnimationManager*)node->getUserObject();
    am->runAnimationsForSequenceNamed(str->c_str());
}
...
void CCBAnimationManager::setAnimatedProperty(const char *pPropName, CCNode *pNode, CCObject *pValue, float fTweenDuration)
{
    if (fTweenDuration > 0)
    {
        ...
        // Animate
        CCActionInterval *tweenAction = getAction(NULL, kf1, pPropName, pNode,true);
    ...
    else
    {
        ...
        else
        {
            ...
            else if (strcmp(pPropName, "visible") == 0)
            {
                ...
                //                CCArray* arr = ((CCBValue*)pValue)->getArrayValue();
                //                if (arr!=NULL && arr->count()==2) {
                //                    CCString* str = (CCString*)arr->objectAtIndex(0);
                //                    CCFloat* f = (CCFloat*)arr->objectAtIndex(1);
                //                    CCLog("setAnimatedProperty : timeline = (%s,%f)",str->getCString(),f->getValue());
                //                }
            }
            else
            {
...
void CCBAnimationManager::runAction(CCNode *pNode, CCBSequenceProperty *pSeqProp, float fTweenDuration)
{
    ...
    if (numKeyframes > 1)
    {
        ...
        for (int i = 0; i < numKeyframes - 1; ++i)
        {
            ...
            bool last = (i == numKeyframes-2);
            
            CCActionInterval *action = getAction(kf0, kf1, pSeqProp->getName(), pNode,last);
            if (action)
            {
                // Apply easing
...
void CCBAnimationManager::runAnimationsForSequenceIdTweenDuration(int nSeqId, float fTweenDuration)
{
    ...
    if (completeAction!=NULL) {
        mRootNode->stopAction(completeAction);
    }
    if (callbackChannelAction!=NULL) {
        mRootNode->stopAction(callbackChannelAction);
    }
    if (soundChannelAction!=NULL) {
        mRootNode->stopAction(soundChannelAction);
    }
    ...
    completeAction = CCSequence::createWithTwoActions(CCDelayTime::create(seq->getDuration() + fTweenDuration),
    ...
    if(seq->getCallbackChannel() != NULL) {
        callbackChannelAction = (CCAction *)actionForCallbackChannel(seq->getCallbackChannel());
        if(callbackChannelAction != NULL) {
            mRootNode->runAction(callbackChannelAction);
        }
    }
    
    if(seq->getSoundChannel() != NULL) {
        soundChannelAction = (CCAction *)actionForSoundChannel(seq->getSoundChannel());
        if(soundChannelAction != NULL) {
            mRootNode->runAction(soundChannelAction);
        }
    }
